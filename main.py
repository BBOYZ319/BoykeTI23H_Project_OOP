...
def on_delete_account():
    ...
    if confirm:
        if delete_account(current_user):
            ...
            current_user = None
            dashboard_frame.winfo_toplevel().destroy()  # Tutup window dashboard
            root.deiconify()  # Tampilkan kembali window login
        else:
...
def refresh_file_list(frame):
    ...
    for filename in files:
        ...
        lbl = ttk.Label(row, text=filename, width=25)  # cukup pakai filename
...
def open_dashboard():
    ...
    ttk.Label(dashboard, text=f"Selamat datang, {current_user.username}",
              font=("Segoe UI", 12, "bold"), foreground="white", background="#1e1e2f").pack(pady=10)
